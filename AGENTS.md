...
| `<VendorPhoto>`  | `src?`, `alt?`, `initials`, `size='sm'\|'md'\|'lg'`, `tone='sage'\|'terracotta'`, `className?` | Vendor headshot — B&W photo (or initials fallback) inside a cream circle, framed by a sage or terracotta square. Used by `<VendorCard>`. | `components/ui/VendorPhoto.tsx`           |
